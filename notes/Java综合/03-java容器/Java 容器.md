...
| size | 键值对数量。 |      
| threshold | size 的临界值，当 if ((size >= threshold) && (null != table[bucketIndex]))） 就必须进行扩容操作 |     
...
### 8. 链表转红黑树
**<font color="red">从 JDK 1.8 开始，一个桶存储的链表长度大于等于 8 时会将链表转换为红黑树,将链表转换成红黑树前会判断，如果当前数组的长度小于 64，那么会选择先进行数组扩容，而不是转换为红黑树</font>**
